...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, device):
        ...
        attn_weights1, cls1_1, _ = self.transformer(x)
        # attn_weights1, cls1_1 = self.transformer(x)

        coords2 = get_coordi_attention(attn_weights1, 1.2, device)
        # coords2 = get_coordi_attention2(attn_weights1, 180, device)
        # print(coords2)
        local_imgs2 = get_local_imgs(x, coords2, device)
        # attn_weights2, feat_cls3_1, feat_patch3_1, feat_cls3_2, feat_patch3_2,feat_cls3_3, feat_patch3_3 = self.trans(local_imgs2.detach())
        attn_weights2, _, cls2_2 = self.transformer(local_imgs2.detach())

        # coords3 = get_coordi_attention(attn_weights2, 1.3, device)
        # # coords2 = get_coordi_attention2(attn_weights1, 180, device)
        # print(coords3)
        # local_imgs3 = get_local_imgs(local_imgs2, coords3, device)
        # # attn_weights2, feat_cls3_1, feat_patch3_1, feat_cls3_2, feat_patch3_2,feat_cls3_3, feat_patch3_3 = self.trans(local_imgs2.detach())
        # attn_weights3, _,_,cls3_3 = self.transformer(local_imgs3.detach())

        # return local_imgs2#, local_imgs3
        # cls = torch.cat((cls1_1, cls2_2), -1)
        return self.part_head(cls1_1), self.part_head(cls2_2), self.part_head(cls2_2), self.part_head(cls2_2)
        # return self.part_head(cls), self.part_head(cls), self.part_head(cls), self.part_head(cls)
        ...
        # else:
        #     return part_logits, part_logits, part_logits, part_logits
    ...
    def load_from(self, weights, logger):
        with torch.no_grad():
            ...
            if posemb.size() == posemb_new.size():
                ...
                posemb = np.concatenate([posemb_tok, posemb_tok.detach(), posemb_grid], axis=1)
